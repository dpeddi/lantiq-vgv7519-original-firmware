...
int main(int argc, char *argv[]) {
    ...
    if (argc != 3) {
        printf("usage: easybox_obfuscate infile outfile.bin.lzma\n");
        return -1;
    }
    ...
    buffer = (unsigned char*) malloc(insize + 8);
    if (buffer == NULL) {
        fputs("memory error", stderr);
        exit(2);
    }
 
    printf("read \t%i bytes\n", fread(buffer + 4, 1, insize , infile));
    ...
    printf("wrote \t%i bytes\n", fwrite(buffer , 1, insize + 4, outfile));
    fclose(outfile);
